...
colnames(stat_tab) <- c('PCR.dedupliated.reads', 'mapped.reads', 'uniquely.mapped.reads', 'crosslink.sites', 'crosslink.sites.all', 'crosslink.sites.FDR<0.05', 'crosslink.sites.clusters')
...
for (i in 1:length(config$samples)) {
  ...
  stat_tab[i, 1:4] <- c(mapping_stat[mapping_stat$V1 == config$samples[i], 3], nrow(x))
}
