...
stat_tab <- matrix(nrow=length(qc$sample), ncol=5)
rownames(stat_tab) <- qc$sample
colnames(stat_tab) <- c('PCR.dedupliated.reads', 'uniquely.mapped.reads', 'crosslink.sites', 'crosslink.sites.FDR<0.05', 'crosslink.sites.clusters')
