...
pdf('occupancy/MeCP2_KO_RBFOX2_KI_SE_RPKM_2_v4.pdf', hei=5)
par(mfrow=c(2, 2))
#par(mar=c(5,4,4,1))
par(mar=c(2,4,1,1))
# enhanced SE events
plot(wt_en_3ss$V1, smooth_occu(wt_en_3ss$V2, step=10), ylim=c(0,0.55), type='l', bty='l', las=2, lwd=1.5, col=col_set[1], ylab='Normalized cDNA counts', xlab='', xaxt='n')
...
abline(h=0)
rect(100, -0.02, 200, 0.02, col='gray30', border = NA)
...
#par(mar=c(5,1,4,4))
par(mar=c(2,1,1,4))
plot(wt_en_5ss$V1, smooth_occu(wt_en_5ss$V2, step=10), ylim=c(0,0.55), type='l', bty='n', yaxt='n', las=2, lwd=1.5, col=col_set[1], ylab='', xlab='', main='', xaxt='n')
...
abline(h=0)
rect(-50, -0.02, 50, 0.02, col='gray30', border = NA)
...
par(mar=c(2,4,1,1))
# silenced SE events
plot(wt_si_3ss$V1, smooth_occu(wt_si_3ss$V2, step=10), ylim=c(0,0.55), type='l', bty='l', las=2, lwd=1.5, col=col_set[3], ylab='Normalized cDNA counts', xlab='', xaxt='n')
...
#par(mar=c(5,1,4,4))
par(mar=c(2,1,1,4))
plot(wt_si_5ss$V1, smooth_occu(wt_si_5ss$V2, step=10), ylim=c(0,0.55), type='l', bty='n', yaxt='n', las=2, lwd=1.5, col=col_set[3], ylab='', xlab='', main='', xaxt='n')
